"""
05 - Transfer Learning con EfficientNetB0
...
def build_augmentation_layer():
    """Augmentation para transfer learning."""
...
def build_transfer_model():
    """EfficientNetB0 con cabeza personalizada."""
    ...
    # Sin fijar training=True explícito: así la augmentation hereda el modo
    # de la llamada al modelo completo y se desactiva automáticamente en
    # evaluate()/predict() (ver misma nota en 04_train_cnn_aug.py).
    x = build_augmentation_layer()(inputs)

    # EfficientNet espera preprocesamiento específico
...
def plot_training_history(history, history_ft, filename):
    """Curvas combinadas: fase congelada + fine-tuning."""
    ...
    fig.suptitle("Transfer Learning (EfficientNetB0) — Curvas de Entrenamiento", fontsize=14)
    fig.tight_layout()
...
def main():
    print("=" * 60)
    print("  ENTRENAMIENTO — Transfer Learning (EfficientNetB0)")
    print("=" * 60)
    ...
    try:
        ...
        # epoch_offset usa las épocas realmente completadas en fase 1
        # (puede ser menor que PHASE1_EPOCHS por early stopping)
        ...
        model.save(MODELS_DIR / "transfer_efficientnet.keras")
        print(f"  → Modelo: {MODELS_DIR / 'transfer_efficientnet.keras'}")

        plot_training_history(history, history_ft, "curvas_transfer_learning.png")

        val_loss, val_acc = model.evaluate(X_val, y_val, verbose=0)
        print(f"\n  Resultado en validación → Loss: {val_loss:.4f}, Accuracy: {val_acc:.4f}")
        print("\n✓ Entrenamiento Transfer Learning completado.")
